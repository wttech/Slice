 ...
 */
package com.cognifide.slice.mapper.util;
...
/**
 * THIS CLASS IS COPY OF MemberUtils from commons-lang v 2.5
 * 
...
abstract class MemberUtils {
	 ...
	 * 
	 * When a public class has a default access superclass with public members, these members are accessible.
	 * Calling them from compiled code works fine. Unfortunately, on some JVMs, using reflection to invoke
	 * these members seems to (wrongly) to prevent access even when the modifer is public. Calling
	 * setAccessible(true) solves the problem but will only work from sufficiently privileged code. Better
	...
	/**
	 * Compare the relative fitness of two sets of parameter types in terms of matching a third set of runtime
	 * parameter types, such that a list ordered by the results of the comparison would return the best match
	 * first (least).
	 * 
	...
	private static float getObjectTransformationCost(Class srcClass, Class destClass) {
		...
		/*
		 * If the destination class is null, we've travelled all the way up to an Object match. We'll penalize
		 * this by adding 1.5 to the cost.
		 */
		if (destClass == null) {
			cost += 1.5f;
		}
